...
const STAGES = [
  ...
  },
  {
    id: "everything",
    lead: "GenAI",
    rest: " is becoming a part of everything.",
    body: "From IDEs to CI pipelines, generation is showing up in every layer of the stack.",
  },
  {
    id: "mundane",
    lead: "GenAI",
    rest: " will execute more mundane development tasks",
    body: "Developers will orchestrate the work of AI agents while focusing on higher-level problem solving",
  },
];

/** Profile card with certifications and skill badges — shown in the first stages. */
  ...
  return (
    <div className="w-[300px] rounded-2xl border border-white/10 bg-[#0d1117] p-5 shadow-2xl sm:w-[360px]">
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <p className="text-[16px] font-semibold text-white">Ada</p>
          <p className="text-[12.5px] text-white/55">Machine Learning Engineer</p>
        </div>
      </div>

      <div className="mt-5 border-t border-white/10 pt-4">
        <p className="text-[12.5px] font-semibold text-white">Certifications</p>
        <div className="mt-3 grid grid-cols-2 gap-2.5">
          {["Machine Learning Engineer", "Data Scientist"].map((cert, index) => (
            <div key={cert} className="rounded-lg border border-white/10 bg-black/40 p-2.5">
              <p className="text-[9.5px] leading-tight text-white/75">{cert}</p>
              <div className="mt-2 h-[3px] w-full rounded-full bg-white/60" />
              <div className="mt-3 flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-white/25" />
                <span className={`h-[3px] rounded-full bg-white/25 ${index ? "w-8" : "w-10"}`} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-5 border-t border-white/10 pt-4">
        <p className="text-[12.5px] font-semibold text-white">Badges</p>
        <div className="mt-4 flex justify-around">
          {[
            { label: "Kubernetes", stars: 3 },
            { label: "PyTorch", stars: 3 },
            { label: "Python", stars: 2 },
          ].map((badge) => (
            <div key={badge.label} data-badge className="flex flex-col items-center gap-1.5">
...
/** Kanban task card reused by the delegation board. */
    ...
    <div className={`rounded-lg bg-[#1b2432] p-3.5 ${dim ? "opacity-45" : ""}`}>
      ...
      <div className="mt-2.5 flex items-center gap-2">
        <span className="inline-flex items-center gap-1 rounded bg-white/10 px-1.5 py-1 text-[10px] text-white/85">
          <span className="text-hr-green">✦</span> AI Help
        </span>
        <span className="inline-flex items-center gap-1 rounded bg-white/10 px-1.5 py-1 text-[10px] text-white/85">
...
/** Backlog → AI delegated board — shown in the final stage. */
function DelegationBoard() {
  return (
    <div className="relative w-[300px] sm:w-[420px]">
      <div data-board="backlog" className="w-[240px] rounded-xl bg-[#141c28] p-3.5 shadow-2xl sm:w-[290px]">
      ...
      >
        <div className="mb-3 flex items-center justify-between px-1">
          <p className="text-[12.5px] text-white/80">AI delegated</p>
          <span className="text-white/45" aria-hidden="true">···</span>
        </div>
        <div className="space-y-2.5">
          <TaskCard
            ...
            dates="May 24 — July 1"
          />
          <TaskCard title="Writing README files" owner="Andreas H." dates="May 12 — June 5" dim />
        </div>
      </div>

      <div data-board="arrow" className="absolute -left-4 bottom-4 sm:-left-10">
        <p className="-rotate-12 text-[15px] italic text-white/85">AI Delegated</p>
...
export default function ChangingDev() {
  ...
  return (
    <section id="changing" ref={root} className="relative h-[340vh] scroll-mt-16">
      <div
        data-panel
        className="sticky top-[var(--frame-top)] flex h-[calc(100vh-var(--frame-top)-var(--frame-gap))] flex-col overflow-hidden bg-white px-5 py-12 sm:px-8"
      >
        <h2
          data-heading
          className="text-center text-[30px] font-semibold leading-tight tracking-tight sm:text-[42px]"
        >
          <span className="text-[#22c55e]">AI Changing</span>
          <br />
